...
routestatus_enum = postgresql.ENUM(
    "active", "suspended", "seasonal", "inactive",
    name="routestatus", create_type=False
)
vehicletype_enum = postgresql.ENUM(
    "14-seater", "32-seater", "52-seater", "electric",
    name="vehicletype", create_type=False
)
operatingstatus_enum = postgresql.ENUM(
    "active", "suspended", "seasonal", "inactive",
    name="operatingstatus", create_type=False
)
aliastype_enum = postgresql.ENUM(
    "abbreviation", "colloquial", "former_name",
    name="aliastype", create_type=False
)
stagetype_enum = postgresql.ENUM(
    "formal", "informal",
...
faretype_enum = postgresql.ENUM(
    "peak", "off_peak", "late_night", "weekend", "public_holiday",
    name="faretype", create_type=False
)
...
alerttype_enum = postgresql.ENUM(
    "short_loop", "delayed", "suspended_temporary", "diversion", "inactive",
    name="alerttype", create_type=False
)
correctionstatus_enum = postgresql.ENUM(
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("operating_status", operatingstatus_enum, nullable=False, server_default="active"),
        sa.Column("safety_rating",    sa.Numeric(2, 1), nullable=True),
        sa.Column("comfort_rating",   sa.Numeric(2, 1), nullable=True),
        sa.Column("is_verified",      sa.Boolean(),    nullable=False, server_default="false"),
        sa.Column("image_url",        sa.String(),     nullable=True),       # ← merged
    ...
    op.create_table(
        "sacco_aliases",
        sa.Column("id",         postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("sacco_id",   postgresql.UUID(as_uuid=True), sa.ForeignKey("saccos.id"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id",                       postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("sacco_id",                 postgresql.UUID(as_uuid=True), sa.ForeignKey("saccos.id"),    nullable=False),
        sa.Column("corridor_id",              postgresql.UUID(as_uuid=True), sa.ForeignKey("corridors.id"), nullable=True),
        sa.Column("origin_stage_id",          postgresql.UUID(as_uuid=True), sa.ForeignKey("stages.id"),   nullable=False),
        sa.Column("dest_stage_id",            postgresql.UUID(as_uuid=True), sa.ForeignKey("stages.id"),   nullable=False),
        ...
        sa.Column("route_status",             routestatus_enum, nullable=False, server_default="inactive"), 
    ...
    op.create_table(
        "transfers",
        sa.Column("id",                postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("leg1_route_id",     postgresql.UUID(as_uuid=True), sa.ForeignKey("routes.id"), nullable=False),
        sa.Column("leg2_route_id",     postgresql.UUID(as_uuid=True), sa.ForeignKey("routes.id"), nullable=False),
        sa.Column("transfer_stage_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("stages.id"), nullable=False),
    ...
    op.create_table(
        "corridor_surges",
        sa.Column("id",           postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("corridor_id",  postgresql.UUID(as_uuid=True), sa.ForeignKey("corridors.id"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("fare_type",           sa.String(50),     nullable=False),
        sa.Column("reported_at",         sa.DateTime(timezone=True), nullable=False),
        sa.Column("status",              correctionstatus_enum, nullable=False, server_default="pending"),
    ...
    op.create_table(
        ...
        sa.Column("resolved_origin_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("resolved_dest_id",   postgresql.UUID(as_uuid=True), nullable=True),
