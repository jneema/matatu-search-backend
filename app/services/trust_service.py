...
async def get_data_confidence(route: Route, db: AsyncSession) -> str:
    ...
    from app.models.fare import FareType
    ...
    result = await db.execute(
        select(AppSettings).where(AppSettings.key == "fare_staleness_high_days")
    )
    ...
    result = await db.execute(
        select(AppSettings).where(AppSettings.key == "fare_staleness_medium_days")
    )
    setting = result.scalar_one_or_none()
    if setting:
